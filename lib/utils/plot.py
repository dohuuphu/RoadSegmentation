...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False):
    ...
    if not is_demo:
        ...
        color_seg_2 = color_area
        color_area[result[1] ==1] = [255, 0, 0]
        color_seg = color_area
        # color_seg_2= color_area[result[0] == 1] = [0, 255, 0]
        # color_seg_2 = color_seg_2[..., ::-1]
    ...
    blank_image[color_mask != 0] = blank_image[color_mask != 0] * 0.5 + color_seg_2[color_mask != 0] * 0.5
    blank_image = blank_image.astype(np.uint8)
    blank_image = cv2.resize(blank_image, (1280,720), interpolation=cv2.INTER_LINEAR)
    # cv2.imwrite(f"/gdrive/MyDrive/Colab_Notebooks/VMprojectDec/src/YOLOP/inference/output/{count}_seg.jpg", blank_image)
